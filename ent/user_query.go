...
func (uq *UserQuery) QueryOwner() *RoleQuery {
...
func (uq *UserQuery) WithOwner(opts ...func(*RoleQuery)) *UserQuery {
	query := &RoleQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].role_roles == nil {
				continue
			}
			fk := *nodes[i].role_roles
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_roles" returned %v`, n.ID)
			}
			for i := range nodes {
